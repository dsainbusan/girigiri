...
| **9/29~10/2** | 시험기간 — 전원 작업 중단 (2026-09-16 확정) |
| **9/28** | 코드 프리즈 (이상적 마감, 9/30→시험 전으로 앞당김) |
| 10/6 | 1차 제출 (문서 6종) |
| 10/7 | 발표 |
| 10/16 | 2차 제출 (시연 영상) |

9/18 이후 새 기능 브랜치를 만들지 않는다. 9/18~9/28은 QA, 버그 수정, 인라인 스타일 정리, 레이어 규칙 소급 정리에 쓴다(시험기간 9/29~10/2는 제외, 자세한 재조정 근거는 CLAUDE.md "개발 체크리스트" 참고).
...
## 담당자 - 폴더 매핑 (CLAUDE.md 역할분담 요약, 2026-09-16 갱신 — 김태훈 이탈 반영)

| 담당자 | 주로 건드리는 영역 |
|---|---|
| 문창호 | security/, authView/(로그인·소셜로그인), role/viewMode 세션 로직, POS json 연동, 할인율 자동계산, 픽업 예약 관리(QR)·노쇼 방지, 판매·폐기 리포트(Excel/PDF), 마이페이지/절약 가계부, **storeView/(재고 등록, 판매/등록 현황 대시보드, 통계 그래프), 공지사항 게시판 관리, 문의 답변/리뷰 답글 (2026-09-16, 김태훈 인수)** |
