...
| 담당자 | 축 | 주요 기능 |
|---|---|---|
| 송보미 (조장) | 공통 · 백엔드 코어 | 소셜로그인, role 라우팅·모드 분기, 예약 로직, 할인율 자동계산, 전체 기획·조율 |
| 송채현 | 공통 DB + 사장님용 | ERD/DB 설계, 재고 등록, POS json 연동, 판매/폐기 대시보드·통계, 판매·폐기 리포트(Excel/PDF) |
| 강노은 | 사장님용 + 발표 | 픽업 예약 관리·노쇼 방지, 결제(PortOne)·QR·영수증 PDF, 게시판, PPT·시연영상 |
| 김태훈 | 일반 유저용 (화면) | 공통 레이아웃(모바일), 상품 상세, 마이페이지, 리뷰, 환경 기여도 시각화 |
| 문창호 | 일반 유저용 (지도·실시간) | 카카오맵 연동, 지도 메인 탐색, 카테고리 필터, 찜하기, 실시간 알림(WebSocket) |
...
### Phase 1: 기초 (우선 착수)
- [ ] 소셜 로그인 (카카오/네이버 OAuth2) — 송보미
- [ ] Role 기반 라우팅 + viewMode 세션 — 송보미
- [ ] 공통 레이아웃 구현 (모바일 고정폭 컨테이너, 헤더/네비) — 김태훈
- [ ] DB 스키마 설계 · 엔티티 구현 · 공통 CRUD — 송채현
- [ ] 카카오맵 연동 (위도/경도 수신) — 문창호

### Phase 2: 핵심 기능
- [ ] 재고 등록 & 관리 — 송채현
- [ ] 실시간 알림 (WebSocket) — 문창호
- [ ] 예약 로직 — 송보미
- [ ] 마이페이지 UI — 김태훈
- [ ] 지도 메인 화면 — 문창호
